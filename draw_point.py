...
if __name__ =="__main__":
	...
	for index in range(len(files)):
		if side == "ask":
			...
			plt.plot(timeArrs[index], askArrs[index], color=colors[len(files) - 1 - index], marker='o', markersize=5, linestyle='None', label=files[index])
		else:
			#plt.plot(timeArrs[index], bidArrs[index], color=colors[len(files) - 1 - index], label=files[index])
			plt.plot(timeArrs[index], bidArrs[index], color=colors[len(files) - 1 - index], marker='o', markersize=5, linestyle='None', label=files[index])
